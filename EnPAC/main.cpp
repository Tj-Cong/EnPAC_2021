...
//        cout<<"-----------------------------------"<<endl;
        ...
        if(printFlag) {
            string filename = "BA_"+propertyid;
            BA.PrintBuchi(filename+".dot");
            string command = "dot -Tpng "+filename+".dot -o "+filename+".png && rm -f "+filename+".dot";
...
//            cout<<"STATE SPACE:"<<graph->nodecount<<endl;
        ...
        if (NUPN || SAFE || PINVAR || LONGBITPLACE) {
            ...
            endtime = clock();
//            cout<<endl;
            cout<<" NODECOUNT "<<bitgraph->nodecount<<" TIME "<<(endtime-starttime)/CLOCKS_PER_SEC
            ...
            each_used_time=product->ModelChecker(propertyid,each_run_time);
            endtime = clock();
            cout<<" NODECOUNT "<<graph->nodecount<<" TIME "<<(endtime-starttime)/CLOCKS_PER_SEC
                <<" MEM "<<product->get_current_mem()<<endl;
            int ret = product->getresult();
...
int main2() {
    cout<<" _______          ______     _____     _____"<<endl;
    cout<<"|  ____|         |  ___ \\   /     \\   / ____|"<<endl;
    cout<<"| /_____  _____  | |___) | |  /_\\  | | |"<<endl;
    cout<<"|  ____| |  _  \\ |  ____/  |  ___  | | |"<<endl;
    cout<<"| /_____ | | | | | |       | |   | | | |____"<<endl;
    cout<<"|______| |_| |_| |_|       |_|   |_|  \\_____|"<<endl;
    cout<<"EnPAC (Enhanced Petri-net Analyser and Checker) "<<endl;
    cout<<"==============================================="<<endl;
    CHECKMEM();
...
int main(int argc,char *argv[])
{
    ...
    else if(argc > 2 && string(argv[2]) != "-p") {
        cerr<<"[WARNING] unknow parameter "<<argv[2]<<endl;
    }
